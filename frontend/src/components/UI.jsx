...
export function Button({ children, onClick, disabled, loading, variant = 'primary', style: extra }) {
  const base = {
    display: 'inline-flex',
    alignItems: 'center',
  ...
  return (
    ...
    >
      <circle cx="12" cy="12" r="10" stroke={color} strokeWidth="3"
        fill="none" strokeDasharray="47" strokeDashoffset="12"
        strokeLinecap="round"
      />
    </svg>
  )
...
export function Slider({ label, value, onChange, min = 0.001, max = 0.1, step = 0.001 }) {
  return (
    <div>
      <div style={{
        display: 'flex', justifyContent: 'space-between', marginBottom: '0.4rem',
      }}>
        <span style={{ fontFamily: 'var(--font-mono)', fontSize: '0.73rem', color: 'var(--text-secondary)' }}>
          {label}
        </span>
        <span style={{ fontFamily: 'var(--font-mono)', fontSize: '0.73rem', color: 'var(--accent-lime)', fontWeight: 600 }}>
          ε = {value.toFixed(3)}
        </span>
      </div>
      <input
        ...
        style={{
          width: '100%', accentColor: 'var(--accent-lime)',
    ...
    <div>
      {label && (
        <div style={{
          ...
          color: 'var(--text-secondary)', marginBottom: '0.4rem', letterSpacing: '0.08em',
        }}>
          {label}
        </div>
      )}
        ...
        {options.map((opt) => (
          <button
            ...
            style={{
              ...
              background: value === opt.value ? 'rgba(184,255,63,0.1)' : 'transparent',
              color: value === opt.value ? 'var(--accent-lime)' : 'var(--text-secondary)',
              fontFamily: 'var(--font-mono)',
              fontSize: '0.73rem',
...
export function Divider() {
  return <div style={{ borderTop: '1px solid var(--border-subtle)', margin: '1.25rem 0' }} />
}
...
export function ResultImage({ src, label, downloadName }) {
  ...
  return (
    <div style={{
      ...
      <img src={fullSrc} alt={label} style={{ width: '100%', display: 'block', maxHeight: 300, objectFit: 'contain', background: 'var(--bg-deep)' }} />
      <button
